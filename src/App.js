...
import EditorComponent from "./editer/editer";
import "./App.css";
...
class App extends React.Component {
  ...
  // react hook. called when component is loaded succesuflly inside dom
  componentDidMount = () => {
    firebase
      .firestore()
  ...
  deleteNote = async note => {
    ...
    if (this.state.selectedNoteIndex === noteIndex) {
      ...
      this.state.notes.length > 1
        ? // minus one because there is one less. when we del a note that is selected we want to deselect the note.
          this.selectNote(
